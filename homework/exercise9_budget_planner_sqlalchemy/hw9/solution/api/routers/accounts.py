...
async def create_account(account_data: AccountRequest) -> dict:
    ...
    created_account = await account_service.add(account)
...
async def list_accounts() -> dict:
    ...
    accounts = await account_service.accounts()
    balances = await asyncio.gather(*[account_service.balance(account.id) for account in accounts])
    result = []
    for account, balance in zip(accounts, balances):
        result.append({
                KEY_ID: account.id,
                KEY_NAME: account.name,
                KEY_BALANCE: str(balance),
...
async def get_balance(account_id: int) -> dict:
    ...
    balance = await account_service.balance(account_id)
...
async def get_net_worth() -> dict:
    ...
    net_worth = await account_service.net_worth()
