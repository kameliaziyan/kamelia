...
class ReportService:
    ...
    async def spending_by_category(self, year: int, month: int) -> dict[str, Decimal]:
        transactions = await self._transaction_service.transactions()
        categories_by_id = await self._get_categories_by_id()
    ...
    async def monthly_summary(self, year: int, month: int) -> dict[str, Decimal]:
        transactions = await self._transaction_service.transactions()
        categories_by_id = await self._get_categories_by_id()
    ...
    async def _get_categories_by_id(self) -> dict:
        categories = await self._category_service.categories()
        categories_by_id = {}
